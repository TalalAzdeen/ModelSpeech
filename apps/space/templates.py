...
#             result = self.builder.create_space(name, description, ram, cpu_cores, disk_space,is_gpu, is_global, bandwidth,self.data)
...
class TemplateSpacePlatformBuilder:
    ...
    def _validate_space_inputs(self, description, ram, cpu_cores, disk_space, bandwidth):
        validations = [
            #(isinstance(name, str) and name, "Name must be a non-empty string."),
            (isinstance(description, str) and description, "Description must be a non-empty string."),
            (isinstance(ram, int) and 1 <= ram <= 64, "RAM must be an integer between 1 and 64 GB."),
        ...
        for valid, msg in validations:
            if not valid:
                self.handle_error(msg, 11.1)

 
  
    def createspace_compoent(self,description, ram=20, cpu_cores=3, disk_space=4,
                     is_gpu=True, is_global=True, bandwidth=4):
        try:

            self._validate_space_inputs(description, ram, cpu_cores, disk_space, bandwidth)
            ...
            result = self.builder.create_space("dd", description, ram, cpu_cores, disk_space,is_gpu, is_global, bandwidth,self.data)
            ...
            if result.get("status") == "success":
                ...
                print(f"Error creating space.:{result}")
                return self.handle_error("Error creating space.", result.get("status_code", 500))

        except Exception as e:
            return self.error_event_handler(e, function_name="createspace_compoent")
             

    def updateSpace_compoent(self,description, ram=20, cpu_cores=3, disk_space=4,
                     is_gpu=True, is_global=True, bandwidth=4, name="azdeen"):
              try:
                  self._validate_space_inputs(description, ram, cpu_cores, disk_space, bandwidth)
                  result = self.builder.update_space(name, description, ram, cpu_cores, disk_space,is_gpu, is_global, bandwidth,self.data)
                  print(result)
                  ...
                  if result.get("status") == "success":
                      ...
                      return  self.handle_error("Error update space.", result.get("status_code", 500))

              except Exception as e:
                  return self.error_event_handler(e, function_name="updateSpace_compoent")
    ...
    def getSspacesId_compoent(self,id):
        ...
        try:
           ...
           if not id:
            ...
            self.handle_error("Failed to fetch spaceId.", 11.1)
            print(f"Fetching data for space with ID: {id}.")
           else:

              result=self.builder.get_data_space_by_id(id)
              if result.get("status") == "success":
                  print("Space created successfully.")
                  return result['data']
              else:
                  self.handle_error("Error creating space.", result.get("status_code", 500))

        except Exception as e:
             return self.error_event_handler(e, function_name="getSspacesId_compoent")
    ...
    def get_data_space_by_id(self,id):
        ...
        try:
           if not id:
             return self.handle_error("Failed to fetch spaceId.", 11.1)
             print(f"Fetching data for space with ID: {id}.")
           else:
              return self.builder.get_data_space_by_id(id)
        except Exception as e:
    ...
    def createapi(self, data=None, language="en"):
        ...
        return SpaceHandler(self).get_router()
    ...
    def createapp(self, data=None, language="en"):
            try:
                ...
                with gr.Column() as service_dashboard:
                    ...
                    print("Building Gradio service dashboard interface.")
                    create_space_compoent(self)
                    #Update_space_compoent(self,"space_6dac9af4a71b408fbe1970212e052a43")
